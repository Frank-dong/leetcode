...
int main(int argc, char* argv[])
{
     ...
     int nums[] = {83, 44, 15, 17, 40, 74, 65, 8, 84, 93};
     ...
     //insert_sort(nums, nums_size);
     //shell_sort(nums, nums_size);
     //merge_sort(nums, nums_size);               //10W个数排序约15ms
     //count_sort(nums, nums_size);               //10W个数排序约2ms
     randix_sort(nums, nums_size);                //10W个数排序约7ms
     stop_time();
